...
import { getGroupDay, type GroupDay } from "@/lib/group";
 ...
 * other — there's a five-minute break, so a class ending at 12:25 is not the
 * same boundary as the one starting at 12:30. Ticking only the starts leaves
 * the right-hand edge of most blocks hanging in open space.
 */
 ...
 * apart at this scale and would print on top of one another.
 *
 * Columns of two beat a dozen scattered positions: each gets roughly double the
 * horizontal room, so nothing crowds and every period time is still printed.
 *
 * The trade: the lower time in a column sits above its partner's gridline
 * rather than its own. The gridlines remain the truth for position — there is
 * one at every boundary — and the labels read as a legend for the column.
 */
            ...
            {group.members.map((member, index) => (
              <li key={member.id} className="flex items-center gap-3">
                <span
                  ...
                  // 5.25rem + the 0.75rem flex gap = the 6rem (ml-24) that the
                  // axis and the common-window bands are offset by, so the
                  // three layers line up exactly.
                  style={{ width: "5.25rem" }}
                >
                ...
                <div className="relative h-7 flex-1 overflow-hidden rounded-md bg-surface-sunk">
                  {/* Period boundaries, drawn before the blocks so a class
                      covers the lines it spans. Every block now starts and ends
      ...
      {group.nearMisses.length > 0 ? (
        <section className="mt-12 border-t border-border pt-8">
          ...
          <p className="mt-2 text-sm text-text-muted">
            With five timetables a perfect overlap is rare, so these are usually the
              ...
              >
                ...
                <span className="flex-1 text-sm text-text-muted">
                  ...
                  <span className="text-text">
                    {gap.blockedBy.map((p) => p.name.split(" ")[0]).join(", ")}
                  </span>
                </span>
